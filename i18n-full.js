(function () {
  ...
  phraseRows.push(
    ...
    ["O PancreAI é um protótipo educacional que sugere alimentos e porções a partir de uma foto e calcula uma estimativa após revisão. Ele não é um dispositivo médico.", "PancreAI is an educational prototype that suggests foods and portions from a photo and calculates an estimate after review. It is not a medical device.", "PancreAI es un prototipo educativo que sugiere alimentos y porciones a partir de una foto y calcula una estimación tras la revisión. No es un dispositivo médico.", "PancreAI est un prototype éducatif qui suggère des aliments et des portions à partir d’une photo et calcule une estimation après vérification. Ce n’est pas un dispositif médical.", "PancreAI ist ein Bildungsprototyp, der anhand eines Fotos Lebensmittel und Portionen vorschlägt und nach der Prüfung eine Schätzung berechnet. Er ist kein Medizinprodukt.", "PancreAI è un prototipo educativo che suggerisce alimenti e porzioni da una foto e calcola una stima dopo la revisione. Non è un dispositivo medico."],
    ...
    ["A foto escolhida é processada no próprio navegador pela IA local e não é enviada a um serviço de análise. O modelo é baixado na primeira utilização e pode ser armazenado no cache. Dados de tratamento e histórico também permanecem no navegador nesta versão.", "The selected photo is processed on this device by local AI and is not sent to an analysis service. The model is downloaded on first use and may be stored in the browser cache. Treatment and history data also remain in the browser in this version.", "La foto elegida se procesa en este dispositivo mediante IA local y no se envía a un servicio de análisis. El modelo se descarga en el primer uso y puede guardarse en la caché del navegador. Los datos de tratamiento y el historial también permanecen en el navegador en esta versión.", "La photo choisie est traitée sur cet appareil par une IA locale et n’est pas envoyée à un service d’analyse. Le modèle est téléchargé lors de la première utilisation et peut être conservé dans le cache du navigateur. Les données de traitement et l’historique restent également dans le navigateur dans cette version.", "Das ausgewählte Foto wird auf diesem Gerät von einer lokalen KI verarbeitet und nicht an einen Analysedienst gesendet. Das Modell wird bei der ersten Verwendung heruntergeladen und kann im Browser-Cache gespeichert werden. Behandlungs- und Verlaufsdaten bleiben in dieser Version ebenfalls im Browser.", "La foto scelta viene elaborata su questo dispositivo dall’IA locale e non viene inviata a un servizio di analisi. Il modello viene scaricato al primo utilizzo e può essere memorizzato nella cache del browser. Anche i dati del trattamento e la cronologia restano nel browser in questa versione."],
    ...
    ["A IA pode omitir, confundir ou estimar incorretamente alimentos e porções. A revisão do usuário é obrigatória, e o protótipo não foi validado para decisões clínicas.", "AI may omit, confuse, or incorrectly estimate foods and portions. User review is required, and the prototype has not been validated for clinical decisions.", "La IA puede omitir, confundir o estimar incorrectamente alimentos y porciones. La revisión del usuario es obligatoria y el prototipo no ha sido validado para decisiones clínicas.", "L’IA peut omettre, confondre ou estimer incorrectement des aliments et des portions. La vérification par l’utilisateur est obligatoire et le prototype n’a pas été validé pour des décisions cliniques.", "Die KI kann Lebensmittel und Portionen auslassen, verwechseln oder falsch einschätzen. Die Prüfung durch den Nutzer ist erforderlich, und der Prototyp wurde nicht für klinische Entscheidungen validiert.", "L’IA può omettere, confondere o stimare in modo errato alimenti e porzioni. La revisione dell’utente è obbligatoria e il prototipo non è stato convalidato per decisioni cliniche."],
    ...
    ["A análise depende da imagem, do modelo local, das correções do usuário e dos dados cadastrados. O uso deste protótipo é educacional e demonstrativo.", "The analysis depends on the image, the local model, user corrections, and the registered data. This prototype is for educational and demonstration use.", "El análisis depende de la imagen, del modelo local, de las correcciones del usuario y de los datos registrados. El uso de este prototipo es educativo y demostrativo.", "L’analyse dépend de l’image, du modèle local, des corrections de l’utilisateur et des données enregistrées. L’usage de ce prototype est éducatif et démonstratif.", "Die Analyse hängt vom Bild, dem lokalen Modell, den Korrekturen des Nutzers und den gespeicherten Daten ab. Dieser Prototyp dient Bildungs- und Demonstrationszwecken.", "L’analisi dipende dall’immagine, dal modello locale, dalle correzioni dell’utente e dai dati registrati. L’uso di questo prototipo è educativo e dimostrativo."],
    ["6. Modo demonstrativo", "6. Demo mode", "6. Modo demostrativo", "6. Mode démonstration", "6. Demomodus", "6. Modalità dimostrativa"],
    ["O modo demonstrativo usa imagens e análises preparadas e só é ativado quando escolhido explicitamente. Uma falha da análise real não é substituída silenciosamente por uma simulação.", "Demo mode uses prepared images and analyses and is enabled only when explicitly selected. A real-analysis failure is never silently replaced with a simulation.", "El modo demostrativo usa imágenes y análisis preparados y solo se activa cuando se elige explícitamente. Un fallo del análisis real no se sustituye silenciosamente por una simulación.", "Le mode démonstration utilise des images et des analyses préparées et n’est activé que lorsqu’il est choisi explicitement. Un échec de l’analyse réelle n’est jamais remplacé silencieusement par une simulation.", "Der Demomodus verwendet vorbereitete Bilder und Analysen und wird nur nach ausdrücklicher Auswahl aktiviert. Ein Fehler der realen Analyse wird niemals stillschweigend durch eine Simulation ersetzt.", "La modalità dimostrativa usa immagini e analisi preparate e si attiva solo quando viene scelta esplicitamente. Un errore dell’analisi reale non viene mai sostituito silenziosamente da una simulazione."],
