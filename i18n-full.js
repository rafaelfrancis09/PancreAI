(function () {
  ...
  const phraseRows = [
    ...
    ["O PancreAI envia a foto da refeição por um backend seguro ao Gemini 2.5 Flash para análise visual real.","PancreAI sends the meal photo through a secure backend to Gemini 2.5 Flash for real visual analysis.","PancreAI envía la foto de la comida mediante un backend seguro a Gemini 2.5 Flash para un análisis visual real.","PancreAI envoie la photo du repas via un backend sécurisé à Gemini 2.5 Flash pour une analyse visuelle réelle.","PancreAI sendet das Mahlzeitenfoto über ein sicheres Backend an Gemini 2.5 Flash zur echten Bildanalyse.","PancreAI invia la foto del pasto tramite un backend sicuro a Gemini 2.5 Flash per una vera analisi visiva."],
  ...
  phraseRows.push(
    ...
    ["A foto é enviada pelo backend ao Gemini 2.5 Flash, que reconhece alimentos e estima porções visuais. A IA pode errar, por isso cada sugestão precisa ser revisada.","The backend sends the photo to Gemini 2.5 Flash, which recognizes foods and visually estimates portions. AI can make mistakes, so every suggestion must be reviewed.","El backend envía la foto a Gemini 2.5 Flash, que reconoce alimentos y estima porciones visuales. La IA puede equivocarse, por eso cada sugerencia debe revisarse.","Le backend envoie la photo à Gemini 2.5 Flash, qui reconnaît les aliments et estime visuellement les portions. L’IA peut se tromper, chaque suggestion doit donc être vérifiée.","Das Backend sendet das Foto an Gemini 2.5 Flash, das Lebensmittel erkennt und Portionen visuell schätzt. KI kann Fehler machen, daher muss jeder Vorschlag geprüft werden.","Il backend invia la foto a Gemini 2.5 Flash, che riconosce gli alimenti e stima visivamente le porzioni. L’IA può sbagliare, quindi ogni suggerimento va controllato."],
  ...
  phraseRows.push(
    ...
    ["A câmera e a galeria recebem fotos reais. O backend envia a imagem ao Gemini 2.5 Flash, que reconhece alimentos e estima porções visuais.","The camera and gallery receive real photos. The backend sends the image to Gemini 2.5 Flash, which recognizes foods and visually estimates portions.","La cámara y la galería reciben fotos reales. El backend envía la imagen a Gemini 2.5 Flash, que reconoce alimentos y estima visualmente las porciones.","L’appareil photo et la galerie reçoivent de vraies photos. Le backend envoie l’image à Gemini 2.5 Flash, qui reconnaît les aliments et estime visuellement les portions.","Kamera und Galerie verwenden echte Fotos. Das Backend sendet das Bild an Gemini 2.5 Flash, das Lebensmittel erkennt und Portionen visuell schätzt.","Fotocamera e galleria ricevono foto reali. Il backend invia l’immagine a Gemini 2.5 Flash, che riconosce gli alimenti e stima visivamente le porzioni."],
  ...
  phraseRows.push(
    ...
    ["Gemini 2.5 Flash via backend","Gemini 2.5 Flash through the backend","Gemini 2.5 Flash mediante el backend","Gemini 2.5 Flash via le backend","Gemini 2.5 Flash über das Backend","Gemini 2.5 Flash tramite il backend"],
    ...
    ["Foto enviada ao backend e analisada pelo Gemini 2.5 Flash","Photo sent to the backend and analyzed by Gemini 2.5 Flash","Foto enviada al backend y analizada por Gemini 2.5 Flash","Photo envoyée au backend et analysée par Gemini 2.5 Flash","Foto an das Backend gesendet und von Gemini 2.5 Flash analysiert","Foto inviata al backend e analizzata da Gemini 2.5 Flash"],
